...
export function getStoryboardRepo(): StoryboardRepo {
    // 未来: if (config.useRemoteDb) return new ApiStoryboardRepo(config.apiBase);
    return localStoryboardRepo;
}

export function getPromptProjectRepo(): PromptProjectRepo {
    // 未来: if (config.useRemoteDb) return new ApiPromptProjectRepo(config.apiBase);
    return localPromptProjectRepo;
}
