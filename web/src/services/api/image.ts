...
/** 降级：通过 DashScope 原生多模态生成接口调用百炼生图模型（qwen-image 系列） */
async function requestImageViaDashScope(config: AiConfig, prompt: string, size?: string, options?: RequestOptions) {
    // 百炼原生接口不支持 CORS，开发环境走 Vite 代理
    const origin = extractOrigin(config.baseUrl);
    const isDashScope = origin.includes("maas.aliyuncs") || origin.includes("dashscope");
    const apiUrl = isDashScope
        ? `/dashscope-proxy/api/v1/services/aigc/multimodal-generation/generation`
        : `${origin}/api/v1/services/aigc/multimodal-generation/generation`;
    const imageSize = size && size !== "auto" ? size.replace("x", "*") : "1024*1024";
    const response = await axios.post(
        ...
        { headers: aiHeaders(config, "application/json"), signal: options?.signal, timeout: 120000 },
    );
...
function extractImageUrls(content: unknown): string[] {
    ...
    if (typeof content === "string") {
        ...
        while ((match = urlRegex.exec(content)) !== null) { if (!urls.includes(match[1])) urls.push(match[1]); }
        return urls;
    }
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    try {
        ...
        // 百炼/DeepSeek 等不完整支持 /responses 的供应商，直接走 /chat/completions
        const lowerBase = requestConfig.baseUrl.toLowerCase();
        const skipResponses = lowerBase.includes("maas.aliyuncs") || lowerBase.includes("dashscope") || lowerBase.includes("deepseek") || lowerBase.includes("aliyuncs");
        if (skipResponses) {
        ...
        try {
            ...
            const status = fallbackError && typeof fallbackError === "object" && "status" in fallbackError ? (fallbackError as { status: number }).status : 0;
