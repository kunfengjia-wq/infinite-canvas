...
export function StoryboardTable({ variant = "bottom", wide = false, maximized = false, onToggleMaximize }: Props) {
    ...
    const fullColumns: ColumnsType<TableRow> = [
        ...
        { title: "场景", dataIndex: "sceneTitle", width: 90, ellipsis: true, render: (v: string) => <span className="text-xs text-stone-500">{v}</span> },
        ...
        { title: "动作", dataIndex: "action", ellipsis: true, render: (v: string, row) => (
            <Input size="small" variant="borderless" value={v} onChange={(e) => updateShot(row.sceneId, row.id, { action: e.target.value })} className="!px-0 text-xs" />
        )},
        { title: "对白", dataIndex: "dialogue", ellipsis: true, width: 140, render: (v: string | undefined, row) => (
            <Input size="small" variant="borderless" value={v || ""} onChange={(e) => updateShot(row.sceneId, row.id, { dialogue: e.target.value || undefined })} placeholder="-" className="!px-0 text-xs text-amber-600 dark:text-amber-400" />
        )},
        { title: "时长", dataIndex: "duration", width: 44, render: (v?: string) => <span className="text-xs text-stone-400">{v || "-"}</span> },
        { title: "画面描述", dataIndex: "visualDescription", ellipsis: true, render: (v: string, row) => (
            <Input size="small" variant="borderless" value={v} onChange={(e) => updateShotDescription(row.sceneId, row.id, e.target.value)} placeholder="待生成" className="!px-0 text-xs text-stone-500" />
        )},
    ];
    ...
    const compactColumns: ColumnsType<TableRow> = [
        ...
        { title: "动作", dataIndex: "action", ellipsis: true, render: (v: string, row) => (
            <Input size="small" variant="borderless" value={v} onChange={(e) => updateShot(row.sceneId, row.id, { action: e.target.value })} className="!px-0 text-[11px]" />
        )},
        { title: "对白", dataIndex: "dialogue", ellipsis: true, width: 100, render: (v: string | undefined, row) => (
            <Input size="small" variant="borderless" value={v || ""} onChange={(e) => updateShot(row.sceneId, row.id, { dialogue: e.target.value || undefined })} placeholder="-" className="!px-0 text-[11px] text-amber-600 dark:text-amber-400" />
        )},
    ];
